...
function extractRevalResult(html) {
  ...
  if (candidates.length > 0) {
    ...
    let studentInfo = '';
    const textOnly = before.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ');
    const siMatch = textOnly.match(/(Seat\s*No\s*:\s*\S+)/i);
    if (siMatch) studentInfo = siMatch[1];
    const nmMatch = textOnly.match(/(Name\s*:\s*[A-Z][a-zA-Z\s]+)/);
    if (nmMatch) studentInfo += '<br>' + nmMatch[1].trim();
    const prnMatch = textOnly.match(/(PRN\s*:\s*\S+)/i);
